...
class GraphRes(torch.nn.Module):
    ...
    def forward(self, data: torch_geometric.data.Batch) -> torch.Tensor:
        ...
        # ✅ FIX: Don't pass edge_index to pooling
        data = self.pool5(data.x, pos=data.pos, batch=data.batch, return_data_obj=True)

        x_sc = data.x.clone()
        data.x = elu(self.conv6(data.x, data.edge_index, data.edge_attr))  # pool5 creates new edge_index
